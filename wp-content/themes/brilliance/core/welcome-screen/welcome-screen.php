...
class Brilliance_Welcome {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_brilliance_dismiss_recommended_plugins', array(
			$this,
			'brilliance_dismiss_recommended_plugins_callback'
		) );

		add_action( 'wp_ajax_brilliance_brilliance_set_frontpage', array(
			$this,
			'brilliance_set_pages'
		) );
	...
	public function brilliance_set_pages() {
		if ( ! empty( $_GET ) ) {
			 ...
			 * Check action
			 */
			if ( ! empty( $_GET['action'] ) && $_GET['action'] === 'brilliance_set_frontpage' ) {
	...
	public function brilliance_welcome_admin_notice() {
		...
		<div class="updated notice is-dismissible">
			...
			<p><a href="<?php echo esc_url( admin_url( 'themes.php?page=cpotheme-welcome' ) ); ?>" class="button"
			      style="text-decoration: none;"><?php _e( 'Get started with Brilliance', 'brilliance' ); ?></a></p>
		</div>
		<?php
	}
	...
	public function brilliance_welcome_style_and_scripts( $hook_suffix ) {
		...
		wp_enqueue_style( 'cpotheme-welcome-screen-css', get_template_directory_uri() . '/core/welcome-screen/css/welcome.css', array(), '1.2.2' );
	...
	public function brilliance_dismiss_required_action_callback() {
		...
		if ( ! empty( $action_id ) ):
			...
			if ( get_option( 'brilliance_show_required_actions' ) ):
				...
				switch ( $_GET['todo'] ) {
					case 'add';
						$brilliance_show_required_actions[ $action_id ] = true;
						break;
					case 'dismiss';
						$brilliance_show_required_actions[ $action_id ] = false;
						break;
				}
			...
			/* create the new option,with false for the specified id */
				...
				if ( ! empty( $brilliance_required_actions ) ):
					foreach ( $brilliance_required_actions as $brilliance_required_action ):
						if ( $brilliance_required_action['id'] == $action_id ):
	...
	public function brilliance_dismiss_recommended_plugins_callback() {
		$action_id = ( isset( $_GET['id'] ) ) ? $_GET['id'] : 0;
		echo $action_id; /* this is needed and it's the id of the dismissable required action */
		if ( ! empty( $action_id ) ):
			/* if the option exists, update the record for the specified id */
			$brilliance_show_recommended_plugins = get_option( 'brilliance_show_recommended_plugins' );
				...
				switch ( $_GET['todo'] ) {
					case 'add';
						$brilliance_show_recommended_plugins[ $action_id ] = false;
						break;
					case 'dismiss';
						$brilliance_show_recommended_plugins[ $action_id ] = true;
						break;
				}
				update_option( 'brilliance_show_recommended_plugins', $brilliance_show_recommended_plugins );
			/* create the new option,with false for the specified id */
		endif;
		die(); // this is required to return a proper result
	}
		...
		<div class="wrap about-wrap epsilon-wrap">
			...
			<h2 class="nav-tab-wrapper wp-clearfix">
				<a href="<?php echo admin_url( 'themes.php?page=cpotheme-welcome&tab=getting_started' ); ?>"
				   class="nav-tab <?php echo $active_tab == 'getting_started' ? 'nav-tab-active' : ''; ?>"><?php echo esc_html__( 'Getting Started', 'brilliance' ); ?></a>
				<a href="<?php echo admin_url( 'themes.php?page=cpotheme-welcome&tab=recommended_actions' ); ?>"
				   class="nav-tab <?php echo $active_tab == 'recommended_actions' ? 'nav-tab-active' : ''; ?> "><?php echo esc_html__( 'Recommended Actions', 'brilliance' ); ?>
				...
				<a href="<?php echo admin_url( 'themes.php?page=cpotheme-welcome&tab=recommended_plugins' ); ?>"
				   class="nav-tab <?php echo $active_tab == 'recommended_plugins' ? 'nav-tab-active' : ''; ?> "><?php echo esc_html__( 'Recommended Plugins', 'brilliance' ); ?></a>
				<a href="<?php echo admin_url( 'themes.php?page=cpotheme-welcome&tab=support' ); ?>"
				   class="nav-tab <?php echo $active_tab == 'support' ? 'nav-tab-active' : ''; ?> "><?php echo esc_html__( 'Support', 'brilliance' ); ?></a>
				<a href="<?php echo admin_url( 'themes.php?page=cpotheme-welcome&tab=features' ); ?>"
				   class="nav-tab <?php echo $active_tab == 'features' ? 'nav-tab-active' : ''; ?> "><?php echo esc_html__( 'Lite vs PRO', 'brilliance' ); ?></a>
			</h2>
