<?php
class invert_lite_breadcrumb_class {
    var $opts;

    function invert_lite_custom_breadcrumb() {
        $this->opts = array(
            'before' => '&nbsp;',
            'after' => '&nbsp; ',
            'delimiter' => '&raquo;'
        ...
        echo '<section class="cont_nav"><div class="cont_nav_inner"><p><a href="' . esc_url(home_url('/')). '">' . __('Home', 'invert-lite') . '</a>';
        ...
        $output = $this->invert_lite_simple_breadcrumb_case($post);
    ...
    function invert_lite_simple_breadcrumb_case($der_post) {
        $markup = $this->opts['before'] . $this->opts['delimiter'] . $this->opts['after'];
        
        if (is_page()) {
            if ($der_post->post_parent) {
                $my_query = get_post($der_post->post_parent);
                //$this->invert_lite_simple_breadcrumb_case($my_query);
        ...
        if (is_author()) {
            ...
            return __('Author', 'invert-lite') . ': ' . $curauth->display_name;
        }

        if (is_tag()) {
            return __('Tag', 'invert-lite') . ': ' . single_tag_title('', false);
        }

        if (is_search()) {
            return __('Search', 'invert-lite');
        }				if (is_404()) {            return __('Error 404', 'invert-lite');        }
...
$invert_breadcumb = new invert_lite_breadcrumb_class();
?>