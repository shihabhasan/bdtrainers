...
class haxel_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'haxel_rp', // Base ID
			__('Haxel Recent Posts with Thumbnails', 'haxel'), // Name
			array( 'description' => __( 'Display your recent posts, with a Thumbnail.', 'haxel' ), ) // Args
		);
	}
	...
	if (isset($instance['title'])) :
		...
		<?php
         ...
         ?>
		         ...
		         ?>
		         <div class='rp-thumb'><a href="<?php the_permalink(); ?>"><img alt="<?php the_title_attribute(); ?>" src="<?php echo get_stylesheet_directory_uri(); ?>/assets/images/nthumb.png"></a></div>
		         <?php
		         endif; ?>	
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'haxel' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'haxel' );
		}
		?>
		<p>
		<label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php _e( 'Title:','haxel' ); ?></label> 
		<input class="widefat" id="<?php echo esc_attr($this->get_field_id( 'title' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		
		<label for="<?php echo esc_attr($this->get_field_id( 'no_of_posts' )); ?>"><?php _e( 'No. of Posts:', 'haxel' ); ?></label>
...
add_action( 'widgets_init', 'register_haxel_widget' );  
function register_haxel_widget() {  
    register_widget( 'haxel_Recent_Posts' );  
}  
